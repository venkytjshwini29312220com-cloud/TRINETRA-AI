...
from app.reports.intelligence_report import (
    IntelligenceReport,
)
from app.reports.criminal_documentary import (
    CriminalDocumentary,
)
...
def generate_documentary_report(
    ...
    try:
        ...
        result = generator.generate(
            ...
            title=payload.title,
            narrative_style=(
                payload.narrative_style
            ),
            include_narrative=(
                payload.include_narrative
            ),
            include_evidence=(
                payload.include_evidence
            ),
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=(
                "The documentary report generator "
                f"does not expose the expected method: {exc}"
            ),
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=(
                f"Documentary report generation failed: "
                f"{exc}"
            ),
        )
