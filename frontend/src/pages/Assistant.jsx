...
export default function Assistant() {
  ...
  const loadLiveState = useCallback(async () => {
    try {
      const [healthData, contextData] = await Promise.all([
        getAssistantHealth(),
        getAssistantContext({
      ...
      setContext(normalizeContext(contextData));
      ...
      setError(
        err?.response?.data?.detail ||
          err?.message ||
          "Unable to retrieve live assistant state."
      );
    } finally {
      ...
      <section className="assistant-telemetry">
        ...
        <div className="assistant-telemetry-card assistant-investigation-card">
          ...
          <div>
            ...
            <strong>LIVE · #{INVESTIGATION_ID}</strong>
          </div>
        </div>
      </section>
