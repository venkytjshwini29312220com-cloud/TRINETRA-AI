...
const getGraphMetric = (graph, ...keys) =>
  numberValue(firstDefined(...keys.map((key) => graph?.[key])));

export default function Analytics() {
  const [investigationId] = useState(DEFAULT_INVESTIGATION_ID);
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    try {
      const [
        ...
        getAnalyticsSummary(investigationId),
        getGraphCentrality(20),
        getGraphConnectivity(),
        getPatterns(investigationId),
      ]);
      ...
      setError(
        err?.response?.data?.detail ||
          err?.message ||
          "Analytics service could not be reached."
      );
    } finally {
      setLoading(false);
    }
  ...
  const investigationRisk = getRiskValue(summaryRisk);
  const investigationRiskLevel = getRiskLevel(summaryRisk, investigationRisk);
    ...
    <div className="tn-page tn-analytics-page">
      ...
      <section className="tn-analytics-kpis">
        <GlassCard className="tn-analytics-kpi tn-kpi-risk">
          ...
          <strong>{loading ? "—" : `${investigationRisk}/100`}</strong>
        ...
        <GlassCard className="tn-analytics-panel tn-risk-panel">
          ...
          <div className="tn-risk-display">
            <div className="tn-risk-ring">
              <div>
                <strong>
                  {entityLoading ? "…" : selectedRisk !== null ? selectedRisk : investigationRisk}
                </strong>
                <span>/100</span>
              </div>
            </div>
